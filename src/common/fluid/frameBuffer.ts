...
export class FrameBuffer {
  ...
  constructor(private _renderingContext: WebGL2RenderingContext, private _dimensions: Dimensions, internalFormat: number, format: number, type: number, param: number) {
    const {
...
export class DoubleFrameBuffer {
  ...
  private _frameBuffer1: FrameBuffer;
  private _frameBuffer2: FrameBuffer;
  ...
  public get read(): FrameBuffer {
    return this._frameBuffer1;
  }
  public set read(frameBuffer: FrameBuffer) {
    this._frameBuffer1 = frameBuffer;
    this._dimensions = { width: frameBuffer.width, height: frameBuffer.height };
  }

  public get write(): FrameBuffer {
    return this._frameBuffer2;
  }
  public set write(frameBuffer: FrameBuffer) {
    this._frameBuffer2 = frameBuffer;
    this._dimensions = { width: frameBuffer.width, height: frameBuffer.height };
  }

  constructor(renderingContext: WebGL2RenderingContext, private _dimensions: Dimensions, internalFormat: number, format: number, type: number, param: number) {
    this._frameBuffer1 = new FrameBuffer(renderingContext, _dimensions, internalFormat, format, type, param);
    this._frameBuffer2 = new FrameBuffer(renderingContext, _dimensions, internalFormat, format, type, param);
  }
